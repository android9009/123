...
function drawString()
{
	...
	if (isOpposite)
	{
		Render.String(screen_size[0] /2 +5, screen_size[1] /2 +40, 0, "OPPOSITE", [ 177, 151, 255, 0 ],3 );
	}
	else if (isNormal)
	{
		Render.String(screen_size[0] /2 +5, screen_size[1] /2 +40, 0, "NORMAL", [ 177, 151, 255, 0 ],3 );
	}
	...
	if(drawLeft)
	{
		...
		if (isDMG) {
			add_y = add_y + 9;
		    Render.String(screen_size[0] /2 +5, screen_size[1] /2 + add_y, 0, "DMG", [ 255, 255, 255, 0 ],3 );
		}
		if (isFD) {
			add_y = add_y + 9;
	...
	{
		Render.String(screen_size[0] /2 +5, screen_size[1] /2 +49, 0, isHideReal ? "HIDE" : "HIDE", [ 209, 139, 230, 0 ],3 );
		Render.String(screen_size[0] /2 +5, screen_size[1] /2 +58, 0, is_DT ? DT : DT, dt_color,3 );
		if (isHIDESHOTS) {
			add_y = add_y + 9;
			Render.String(screen_size[0] /2 +5, screen_size[1] /2 + add_y, 0, "ONSHOT", [ 124, 195, 13, 0 ],3 );
...
function onCreateMove()
{
	...
	if (isHideRealActive) {		
		if (Global.Tickcount() > oldTick + 17)  {
			backWasPressed = false;
			rightWasPressed = false;
			leftWasPressed = false;
			upWasPressed = false;
