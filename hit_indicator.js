...
function round_prestart() {
    if( UI.GetValue("Misc", "JAVASCRIPT", "[HMR] Reset on round start") ) {
...
function main() {
    ...
    if(Global.GetMapName() == "" || !UI.GetValue("Misc", "JAVASCRIPT", "[HMR] Hit/Miss Ratio"))
        return;
    ...
    Render.String(x, y + (text_size[1]-12)*3, 0, "hit: " + shots.hit, [255, 255, 255, 255], font_size);

    Render.String(x, y + (text_size[1]-12)*1.6, 0, "miss: " + shots.missed, [255, 255, 255, 255], font_size);

    if(shots.fired > 0) {

        Render.String(x, y + (text_size[1]-12)*4.5, 0, shots.hit + " / " + shots.fired + " = " + Math.round(shots.hit_chance) + "%", [255, 255, 255, 255], font_size);

        Render.String(x, y + (text_size[1]-12)*6, 0, shots.missed + " / " + shots.fired + " = " + Math.round(shots.miss_chance) + "%", [255, 255, 255, 255], font_size);
