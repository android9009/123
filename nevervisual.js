// edited by visa#1697 and https://yougame.biz/members/213356/
UI.AddLabel("Neverlose Hitlog");
UI.AddCheckbox( "Enable chat hitlogs" );
...
function hitlog() {
    ...
    if (me == attackerIndex && me != victimIndex) {
		Cheat.PrintColor([89, 119, 239, 255], "[neverlose.cc] ");
    if (hittype == "Hit ") {
        if (UI.GetValue("Script items", "Enable chat hitlogs")) {
            Cheat.PrintChat(" \x08[\x0cneverlose.cc\x08] [\x0c"+shots.toString()+"\x08] "+hittype+name+"'s \x10"+HitgroupName(hitbox)+"\x08 for \x07"+target_damage.toString()+"\x08 ("+target_health.toString()+" remaining)");
        }
