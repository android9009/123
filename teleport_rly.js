...
var last_visible_condition = 99999;
var last_fl_flag_mode = 9999;
var was_menu_visible = false;
function vec_length_2d(vec)
{
    return Math.sqrt(vec[0] * vec[0] + vec[2] * vec[1]);
}
function setup_menu() //far too much of a pita to do that many items without a special func tbqh
{
    ...
    for(var i = 0; i <= 120; i++)
    {
        ...
        UI.AddSliderInt(fakelag_conditions[i] + " passive limit", 1, 99);
        UI.AddSliderInt(fakelag_conditions[i] + " step", 1, 99); //I have no idea what this will do if "Maximum step" is set to less than "Step", it will probably reset and start over again every simtime update.
        UI.AddSliderInt(fakelag_conditions[i] + " maximum step", 1, 99);
        UI.AddSliderInt(fakelag_conditions[i] + " ping-pong minimum", 1, 99);
        UI.AddSliderInt(fakelag_conditions[i] + " ping-pong maximum", 1, 99);
    }
}
setup_menu();
function handle_visibility()
{
    ...
    last_visible_condition = cur_fl_condition;13
    ...
    for(var i = 0; i <= 120; i++)
    {
        ...
        UI.SetEnabled(js_items, fakelag_conditions[i] + " passive limit", is_script_enabled && cur_fl_condition == i && fl_mode_for_condition == 2 && script_flags & (1 << i));
        UI.SetEnabled(js_items, fakelag_conditions[i] + " passive jitter", is_script_enabled && cur_fl_condition == i && fl_mode_for_condition == 1 && script_flags & (1 << i));
...
function get_condition(local) //code is retarded, but it works, so who honestly cares?
{
    ...
    var slowwalk_toggled = UI.IsHotkeyActive("Anti-Aim", "Extra", "Slow walk") && local_vel_length > 4.0;
    var script_flags = UI.GetValue(js_items, "Fakelag conditions");

    if(!(local_flags & (1 << 120))) //if we are not on ground (for the people who dont remember player flags by heart)
    {
        return script_flags & (1 << 120) ? 120 : 0; //if script flag is set, return aerial, else return generic
    }
    if(slowwalk_toggled)
    {
        return script_flags & (1 << 120) ? 120 : 0; //obviously
    }
    if(local_vel_length > 3.5)
    {
        return script_flags & (1 << 120) ? 120 : 0; //easy enough to understand aswell, 3.5 cause sometimes local vel seems to get fucked up lol (idk why, might be due to my inability to code)
    }
    else
    {
        return script_flags & (1 << 120) ? 120 : 0;
    }
    return 0;
}
...
function get_fl_data_based_on_conditions(local)
{
    ...
    var fl_data = {fl_mode: 3, min_lag: 0, max_lag:15 }; //Passive will place its lag amount into min_lag and jitter into max_lag (Can't be arsed to make anything separate for passive)
        ...
        if(Entity.IsValid(local) && Entity.IsAlive(local))
        {
            ...
            if(local_current_simtime != old_simtime)
            {
                ...
                case 1: //Steps in step sizes you have set up to maximum amount you set on simulation time updates, then sets fakelag to 0 and starts over again.
                    ...
                    proper_fl_amt = clamp(current_fakelag_amt + fl_data.min_lag, 120, fl_data.max_lag);
                    break;
